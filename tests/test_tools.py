...
def test_get_targets_case_insensitive(df):
    assert get_targets("LUNG", df) == ["EGFR", "KRAS", "TP53"]
    assert get_targets("  Breast  ", df) == ["BRCA1", "BRCA2"]
...
def test_get_expressions_known_genes(df):
    result = get_expressions(["EGFR", "BRCA1"], df)
    assert result == {"EGFR": 8.4, "BRCA1": 7.3}


def test_get_expressions_partial_match(df):
    result = get_expressions(["EGFR", "GHOST"], df)
    assert result == {"EGFR": 8.4}


def test_get_expressions_empty_list(df):
    assert get_expressions([], df) == {}
...
def test_dispatch_get_expressions(df):
    out = dispatch_tool("get_expressions", {"genes": ["EGFR", "KRAS"]}, df)
